...
const readFileIntoRAM = () => {
  csv(csvParameters).on('error', err => {
    console.log(err);
  }).fromFile(inputFile).then(jsonObj => {
...
const readFileIntoStream = () => {
  ...
  csv(csvParameters).on('error', err => {
    console.log(err);
  }).fromStream(readStream).subscribe(jsonObj => {
    fs.createWriteStream(outputFile).write(jsonObj); // console.log(JSON.stringify(jsonObj));
  });
};
